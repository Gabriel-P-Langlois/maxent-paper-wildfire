function [mat_annual,pprior,Ed,n0,n1,name_features,...
    ind_nan_months,groups] = prepare_wildfire_data
% PREPARE_WILDFIRE_DATA
%
% This function extract all the relevant wildfire data for the regularized
% maximum entropy estimation problem
%
% Requisite data files:
%   - clim_fire_freq_12km_w2020_data.h5
%   - pred_fire_masked_prob_all_mons.h5
%
...
%
% Output:
%   - mat_annual: An N x p feature matrix (N = samples, p = features)
%   - pprior: An N x 1 vector (prior probability)
%   - Ed: A p x 1 vector  (empirical average of the features)
%   - n0: Number of background-only data points
%   - n1: Number of presence-only data points
%   - name_features:  Cell data structure containing the name of the
%                     features
%   - ind_nan_months: Grid cells in the wildfire data to ignore (correspond
%                     to gridcells in the ocean
...
mat_annual =  zeros(length(data_info(:,1))/nb_years,length(amat(1,:)));
for i=1:1:nb_months
    j = 1 + rem(i-1,12);
    range = (1 + (j-1)*nb_spatial_points):(j*nb_spatial_points);
    mat_annual(range,:) = mat_annual(range,:) + ... 
...
mat_annual = mat_annual/double(nb_years);
...
% Scale the features
max_amat_annual = max(mat_annual);
min_amat_annual = min(mat_annual);
mat_annual = (mat_annual - min_amat_annual)./...
