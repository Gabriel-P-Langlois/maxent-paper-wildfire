...
% provided by JB using gradient descent with wolfe linear search
...
%% Script for the algorithm
disp(' ')
disp('Algorithm: Gradient descent with Wolfe line search')
...
for i=1:1:l_max
    ...
    [sol_gd_w(:,i+1),num_iter_tot_reg] = gd_wolfe_solver(sol_gd_w(:,i),t,A,(1/L12_sq),Ed,max_iter);   
...
%% Solver
function [sol_w,num_iter_tot] = gd_wolfe_solver(w,lambda,A,tau,Ed,max_iter)
% Input variables:
...
% TODO:
% 1) Get optimal stepsize (aka eigenvalue)
% 2) Fix tracking of stepsize... there is something weird going on
% 3) Optimize wolfe step size search...
...
for k=1:1:max_iter
    ...
    [fk,gradk] = primal_eval(w,lambda,A,Ed);
    alpha = 2;
    factor = 0.9;
    c1 = 1e-04; c2=0.9;
    ...
    for l=1:1:6
       factor = factor^l;
       step = (alpha*factor)*step_old;
       [fkmod,gradkmod] = primal_eval(w-step*gradk,lambda,A,Ed);
       
       % Strong Wolfe conditions
       cond1 = (fkmod) <= (fk - c1*step*(norm(gradk)^2));
       if(cond1)
           cond2 = abs(gradk'*gradkmod) <= c2*(norm(gradk)^2);
           if(cond2)
               step_old = step;
               break;
           end
    ...
    % AGD with fixed step
    wplus = y - step*grad_primal(y,lambda,A,Ed);
...
sol_w = wplus;
    ...
    function [f,grad] = primal_eval(w_prob,lambda,A,Ed)
        % INEFFICIENT
        % Compute gradient of the log term
        x = A*w_prob; [a,ind] = max(x);
        p = exp(x-a);
        grad = p/sum(p); grad = A'*grad;
        
        % Evaluate gradient of the primal problem
        grad = grad + lambda*w_prob - Ed;
        
        % Evaluate the primal problem
        p(ind) = [];
        s = sum(p);
        f = 0.5*lambda*(norm(w_prob)^2) + ...
            + (a + log1p(s) - log(length(x))) - Ed'*w_prob;
    end
end